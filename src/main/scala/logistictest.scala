...
case class Iris(features: org.apache.spark.mllib.linalg.Vector, label: Int)
case class TestIris(features: org.apache.spark.mllib.linalg.Vector)
object logistictest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //1. 加载数据 17个数据
    val data = sc.textFile("d://sparktest/ubi/trainData.csv").map(_.split(","))
    ...
    //2.过滤数据--test
    dataDF.registerTempTable("ceshi");


     val df = sqlContext.sql("select * from ceshi where label != 2 ")
    ...
    val lrPipelineModel = lrPipeline.fit(df)
    ...
    //加载测试数据
    //1. 加载数据 17个数据
    val testdatap = sc.textFile("d://sparktest/ubi/testDataP.txt").map(_.split("\t"))
      .map(p => TestIris(Vectors.dense(
        p(3).toDouble, p(4).toDouble, p(5).toDouble, p(6).toDouble,
        p(7).toDouble, p(8).toDouble, p(9).toDouble, p(10).toDouble, p(11).toDouble)
      )
      )

    val testdataDF = sqlContext.createDataFrame(testdatap)


    val lrPredictions = lrPipelineModel.transform(testdataDF)


    //打印预测的列
    //​ 最后我们可以输出预测的结果，其中select选择要输出的列，collect获取所有行的数据，用foreach把每行打印出来。
    // 其中打印出来的值依次分别代表该行数据的真实分类和特征值、预测属于不同分类的概率、预测的分类。
     lrPredictions.select("predictedLabel", "features", "probability").collect().foreach {
      println
    }
